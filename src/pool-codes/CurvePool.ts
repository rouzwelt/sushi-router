...
import { CurveMultitokenPool, CurvePool, MultiRoute, RouteLeg } from "./../tines";
...
export class CurvePoolCode extends PoolCode {
  ...
  override getSwapCodeForRouteProcessor4(leg: RouteLeg, _: MultiRoute, to: string): string {
    ...
    const [index0, index1] =
      this.pool instanceof CurveMultitokenPool ? [this.pool.index0, this.pool.index1] : [0, 1];
