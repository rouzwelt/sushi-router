...
export class CurveMultitokenPool extends RPool {
  ...
  calcOutByIn(amountIn: number, direction: boolean): { out: number; gasSpent: number } {
    if (direction) {
      ...
      const out =
        -this.flow1 - this.core.calcOutDiff(amountIn - this.flow0, this.index0, this.index1);
      ...
      const out =
        -this.flow0 - this.core.calcOutDiff(amountIn - this.flow1, this.index1, this.index0);
  ...
  calcInByOut(amountOut: number, direction: boolean): { inp: number; gasSpent: number } {
    if (direction) {
      const inp =
        this.core.calcOutDiff(-amountOut - this.flow1, this.index1, this.index0) + this.flow0;
      console.assert(inp >= 0, "CurveMultitokenPool.calcInByOut Unexpected output value 0");
      return { inp, gasSpent: SWAP_GAS_COST };
    } else {
      const inp =
        this.core.calcOutDiff(-amountOut - this.flow0, this.index0, this.index1) + this.flow1;
...
export class CurveMultitokenCore {
  ...
  diffToAbsolute(diff: number, i: number): bigint {
    return (
      BigInt(Math.round(this.flowExtToInt(diff + this.currentFlow[i]) * this.rates[i])) +
      (this.reserves[i] * this.ratesBN18[i]) / E18
    );
  }
  absoluteToDiff(abs: bigint, i: number): number {
    return (
      this.flowIntToExt(Number((abs * E18) / this.ratesBN18[i] - this.reserves[i])) -
      this.currentFlow[i]
    );
  }
