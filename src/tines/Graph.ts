...
export class Edge {
  ...
  applySwap(from: Vertice) {
    ...
    const outPrev = this.direction ? this.amountOutPrevious : -this.amountOutPrevious;
    const to = from.getNeibour(this);
    const inInc = from === this.vert0 ? from.bestIncome : -(to as Vertice).bestIncome;
    const outInc = from === this.vert0 ? (to as Vertice).bestIncome : -from.bestIncome;
    const inNew = inPrev + inInc;
    const outNew = outPrev + outInc;
    console.assert(inNew * outNew >= 0);
    if (inNew >= 0) {
...
export class Graph {
  ...
  setPricesStable(
    ...
    function addVertice(v: Vertice, price: number) {
      ...
      const newEdges = v.edges
        ...
        .filter(
          ([liquidity, e]) => liquidity >= minLiquidity || e.pool.alwaysAppropriateForPricing(),
        );
  ...
  setPricesStableInsideChain(from: Vertice, price: number, gasPrice: number) {
    ...
    function addVertice(v: Vertice, price: number, gasPrice: number) {
      ...
      const newEdges = v.edges.filter((e) => {
        ...
        return (
          newV?.token.chainId === v.token.chainId && !processedVert.has(v.getNeibour(e) as Vertice)
        );
      });
  ...
  findBestPathExactIn(
    ...
    for (;;) {
      ...
      closestVert.edges.forEach((e) => {
        ...
        if (e.pool.address === (closestVert as Vertice).bestSource?.pool.address) return;
  ...
  updateLegsAmountOut(legs: RouteLeg[], amountIn: number): number {
    ...
    legs.forEach((l) => {
      ...
      const edge = (vert as Vertice).edges.find((e) => e.pool.uniqueID() === l.uniqueId);
      ...
      const routerPortion =
        Math.round(l.swapPortion * ROUTER_DISTRIBUTION_PORTION) / ROUTER_DISTRIBUTION_PORTION;
