...
function getEquilibriumFee(
  ...
  if (afterBalance >= safeZoneMax) {
    ...
    if (beforeBalance >= safeZoneMin) {
      ...
      // part 2
      eqFee =
        eqFee + getTrapezoidArea(LAMBDA_2, LAMBDA_1, safeZoneMin, ZERO, safeZoneMin, afterBalance);
    } else {
      // only in danger zone
      // part 2 only
      eqFee =
...
function getProtocolAndLpFee(
  ...
  const isAboveIdeal =
    state.currentBalance - amountSD > (state.idealBalance * SIXTY_PERCENT) / DENOMINATOR;
