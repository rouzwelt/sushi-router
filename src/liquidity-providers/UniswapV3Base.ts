...
import { computeSushiSwapV3PoolAddress } from "../pool-codes/UniV3Pool";
...
export abstract class UniswapV3BaseProvider extends LiquidityProvider {
  ...
  async fetchPoolsForToken(
    ...
    const wordList = existingPools.flatMap((pool, i) => {
      ...
      return Array.from({ length: maxIndex - minIndex + 1 }, (_, i) => minIndex + i).flatMap(
        (j) => ({
          chainId: this.chainId,
  ...
  getStaticPools(t1: Token, t2: Token): StaticPool[] {
    ...
    const allCurrencyCombinationsWithAllFees: [Type, Type, SushiSwapV3FeeAmount][] =
      currencyCombinations.reduce<[Currency, Currency, SushiSwapV3FeeAmount][]>(
        (list, [tokenA, tokenB]) => {
    ...
    allCurrencyCombinationsWithAllFees.forEach(([currencyA, currencyB, feeAmount]) => {
      if (currencyA && currencyB && feeAmount) {
        ...
        filtered.push(
          tokenA.sortsBefore(tokenB) ? [tokenA, tokenB, feeAmount] : [tokenB, tokenA, feeAmount],
        );
      }
    });
