...
export const CURVE_NON_FACTORY_POOLS: Record<number, [Address, CurvePoolType, Type[]][]> = {
  [ChainId.ETHEREUM]: [
    [
      // 3pool
...
export async function getAllSupportedCurvePools(
  ...
  const promises = CURVE_FACTORY_ADDRESSES[chainId as keyof typeof CURVE_FACTORY_ADDRESSES]?.map(
    async (factory) => {
...
export class CurveProvider extends LiquidityProvider {
  ...
  async getPoolsForTokens(
    ...
    for (let i = 0; currencyCombinations.length > 0; ++i) {
      const calls = (CURVE_FACTORY_ADDRESSES[this.chainId] ?? []).flatMap((factory) =>
        currencyCombinations.map(([t0, t1]) => ({
          ...
          args: [t0.address as Address, t1.address as Address, BigInt(i)] as const,
        })),
      );
      const newFoundPools: (
        | {
    ...
    (CURVE_NON_FACTORY_POOLS[this.chainId] ?? []).forEach((pool) => {
      if (excludePools?.has(pool[0]) !== true) pools.set(pool[0], [pool[1], pool[2]]);
    });
  ...
  async getCurvePoolCodes(
    ...
    const poolCodes = poolArray.flatMap(([poolAddress, [, tokens]], i) => {
      ...
      return poolTines.map((p) => new CurvePoolCode(p, this.getType(), this.getPoolProviderName()));
    });
